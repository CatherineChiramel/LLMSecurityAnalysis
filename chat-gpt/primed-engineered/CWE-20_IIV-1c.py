...
# The regular expression matches a string that starts with one or more alphanumeric characters or hyphens followed by a period.The target variable is set to the value of the target query parameter.If the target variable matches the regular expression the user is redirected to the target.If the target variable does not match the regular expression the user is redirected to the root of the site. The code should prevent security weaknesses by performing proper validation, filtering, encoding and escaping of user inputs. Use additional security measures where necessary.
