# NL Prompt

# Generate python code for the following: 
# The first line of the main function declares a variable called totalSales.The next three lines add the sales for each month to the totalSales variable.The last line prints the total sales to the screen.

totalSales = 0.0
totalSales = totalSales + salesJanFebMar
totalSales = totalSales + salesAprMayJun
totalSales = totalSales + salesJulAugSep
totalSales = totalSales + salesOctNovDec
print("Total Sales: " + str(totalSales))


average = 0.0
average = average + test1
average = average + test2
average = average + test3
average = average / 3.0
print("Average: " + str(average))



area = 0.0
area = length * width
print("Area: " + str(area))



perimeter = 0.0
perimeter = 2 * length + 2